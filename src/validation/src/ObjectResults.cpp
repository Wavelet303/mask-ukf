...
)
{
    ...
    for (auto& metric_item : metric_)
    {
        ...
        for (auto video_name : test_set)
        {
            ...
            // const SingleVideoResult& video_results = results_.at(video_name);
        ...
        if (metric_item.first == "ADD-S")
        {
            ...
            // Print AUC (0.1 m)
            std::cout << "AUC (0.1 m) (" << metric_item.first << ") : " << evaluateAUC(errors_, 0.1) * 100.0 << " ";
            std::cout << "{";
...
)
{
    ...
    for (auto name : metrics)
    {
        ...
        if (metric_name == "ADD-S")
            ...
            metric_.emplace(metric_name, std::unique_ptr<MetricRMSSymmetry>(new MetricRMSSymmetry(object_symmetry_axis)));
        else if (metric_name == "RMSV")
        {
            if (object_has_symmetry)
                metric_.emplace(metric_name, std::unique_ptr<MetricRMSV>(new MetricRMSV(object_symmetry_axis)));
            else
                metric_.emplace(metric_name, std::unique_ptr<MetricRMSV>(new MetricRMSV()));
        }
        else if (metric_name == "FPS")
            metric_.emplace(metric_name, std::unique_ptr<MetricEmpty>(new MetricEmpty()));
        else
        ...
        // std::cout << log_ID_ << "::ctor. Info: successfully loaded metric " << name << "." << std::endl;
    }
    ...
    for (auto& metric_item : metric_)
    {
        ...
        std::unordered_map<std::string, double> partials_rmse_position;
        std::unordered_map<std::string, double> partials_rmse_orientation;
        std::unordered_map<std::string, double> partials_rmse_v;
        std::unordered_map<std::string, double> partials_rmse_w;
        std::unordered_map<std::string, double> partials_fps;
        ...
        for (auto video_name : test_set)
        {
            ...
            double rmse_position_part = 0;
            double rmse_orientation_part = 0;
            double rmse_v_part = 0;
            double rmse_w_part = 0;
            double fps_part = 0;
            ...
            for (std::size_t frame = 1 + skip_frames; frame <= video_results.getNumberFrames(); frame++)
            {
                ...
                VectorXd gt_vel = video_results.getGroundTruthVelocity(frame, 1 + skip_frames);
                VectorXd est_vel = video_results.getObjectVelocity(frame);

                VectorXd error;
                if (metric_item.first == "RMSV")
                    error = metric_item.second->evaluate(est_vel, gt_vel, gt_pose);
                else
                ...
                else if(metric_item.first == "RMSV")
                {
                    // RMSE for velocities
                    rmse_v_ += error(0);
                    rmse_w_ += error(1);
            ...
            {
                partials_rmse_v[video_name] = std::sqrt(rmse_v_part / double(counter_part));
                partials_rmse_w[video_name] = std::sqrt(rmse_w_part / double(counter_part));
            }
            else if(metric_item.first == "FPS")
            {
                partials_fps[video_name] = fps_part / double(counter_part);
        ...
        {
            std::cout << "RMSE (v): " << std::setprecision(3) << std::sqrt(rmse_v_ / double(counter_)) * 100.0 << " ";
            std::cout << "{";
            for (auto partial : partials_rmse_v)
                std::cout << partial.first << ": " << std::setprecision(3) << partial.second * 100.0 << " ";
            std::cout << "}" << std::endl;

            std::cout << "RMSE (w): " << std::setprecision(3) << std::sqrt(rmse_w_ / double(counter_)) * 180.0 / M_PI << " ";
            std::cout << "{";
            for (auto partial : partials_rmse_w)
                std::cout << partial.first << ": " << std::setprecision(3) << partial.second * 180.0 / M_PI << " ";
            std::cout << "}" << std::endl;
        }
        else if(metric_item.first == "FPS")
...
ObjectResults::ObjectResults(const std::vector<std::unique_ptr<const ObjectResults>>& results_set, const std::vector<std::string>& metrics, const double& max_error)
{
    ...
    for (const std::string& metric_name : metrics)
    {
        if (metric_name == "ADD-S")
        {
            ...
            std::cout << "AUC (0.1 m) (" << metric_name << ") : " << evaluateAUC(total_errors, 0.1) * 100.0 << std::endl;
        }
        ...
        {
            std::size_t counter = 0;
            double total_rmse_v = 0;
            double total_rmse_w = 0;
            for (auto& result : results_set)
            {
                double rmse_v;
                double rmse_w;
                std::size_t number_samples;
                std::tie(number_samples, rmse_v, rmse_w) = result->getRMSEV();
            ...
            std::cout << "RMSE (v): " << std::setprecision(3) << std::sqrt(total_rmse_v / double(counter)) * 100.0 << std::endl;
            std::cout << "RMSE (w): " << std::setprecision(3) << std::sqrt(total_rmse_w / double(counter)) * 180.0 / M_PI << std::endl;
        }
        else if(metric_name == "FPS")
        {
            std::size_t counter = 0;
...
std::tuple<std::size_t, double, double> ObjectResults::getRMSEV() const
{
    return std::make_tuple(counter_, rmse_v_, rmse_w_);
}
