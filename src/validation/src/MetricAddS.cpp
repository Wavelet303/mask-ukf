...
#pragma omp parallel for
    ...
    /* In alternative, you can find the clostest points by exhaustive research using the following. */
    // for (std::size_t i = 0; i < ground_truth_model.cols(); i++)
    // {
    //     std::size_t min_index;
    //     (estimated_model.colwise() - ground_truth_model.col(i)).colwise().norm().minCoeff(&min_index);
    //     estimated_model_symmetric.col(i) = estimated_model.col(min_index);
    // }
