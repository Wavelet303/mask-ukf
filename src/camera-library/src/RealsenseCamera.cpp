...
std::pair<bool, MatrixXf> RealsenseCamera::getDepthImage(const bool& blocking)
{
    ...
    cv::Mat image = yarp::cv::toCvMat(*image_in);
    Map<Eigen::Matrix<float, Dynamic, Dynamic, RowMajor>> depth(image.ptr<float>(), image.rows, image.cols);
